...
class DDBCharacter(BaseModel):
    ...
    def calculate_saving_throw(self, index: int):
        self.accessed_applied_modifiers.add(f"{stat_name[index]}-saving-throws")
    ...
    def calculate_weapon_stats(self, weapon: Weapon) -> list[int]:
            ...
            for token in parse_formula_tokens(formula):
                ...
                if token == "(":
                    ...
                    if token.isdigit():
                        ...
                        next_f = lambda l: l(
                            self.calculate_mod(short_stat_lookup[token.split(":")[-1]])
                        )
        ...
        return (
            ...
            + (
                self.get_applied_modifier(
                    "hit-points-per-level", self.calculate_mod(CONSTITUTION)
                )
                * self.levels
            )
        )
    ...
    @property
    def unarmed_stats(self) -> list[int]:
    ...
    def unarmed_modifier(self) -> int:
        ...
        if feature is not None:
            return max(self.calculate_mod(DEXTERITY), self.calculate_mod(STRENGTH))
        return self.calculate_mod(STRENGTH)
...
short_stat_lookup = {n[:3]: i for i, n in stat_name.items()}
skills = {
