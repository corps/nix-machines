...
showSyntax() {
  echo "up-rebuild [--help] {build | switch }" >&2
...
if ! [ "$action" = list -o "$action" = rollback ]; then
  ...
  systemConfig="$(nix-build '<up>' ${extraBuildFlags[@]} -A system)"
fi

if [ "$action" = list -o "$action" = rollback ]; then
  if [ "$USER" != root -a ! -w $(dirname "$profile") ]; then
    sudo -i nix-env -p $profile ${extraProfileFlags[@]}
  else
...
if [ "$action" = switch ]; then
  if [ "$USER" != root -a ! -w $(dirname "$profile") ]; then
    sudo -i nix-env -p $profile --set $systemConfig
  else
