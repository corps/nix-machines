...

{
  options = {
    ...
    system.activationScripts = mkOption {
      ...
      description = ''
        ...
        <command>wsl-rebuild</command>, it's important that they are
        idempotent and fast.
      '';
    };
  ...
  config = {
    ...
    system.activationScripts.script.text = ''
      ...
      # (which is a symlink to the store), /nix/current-system is still
      # used as a garbage collection root.
      ln -sfn "$(readlink -f "$systemConfig")" /nix/current-system

      # Prevent the current configuration from being garbage-collected.
      ln -sfn /nix/current-system /nix/var/nix/gcroots/current-system
