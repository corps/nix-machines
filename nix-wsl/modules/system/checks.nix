...
let
  ...
  nixPath = ''
    wslConfig=$(NIX_PATH=${concatStringsSep ":" config.nix.nixPath} nix-instantiate --eval -E '<wsl-config>' || echo '$HOME/.nixpkgs/wsl-configuration.nix') || true
    if ! test -e "$wslConfig"; then
        echo "[1;31merror: Changed <wsl-config> but target does not exist, aborting activation[0m" >&2
        echo "Create $wslConfig or set nix.nixPath:" >&2
        echo >&2
        echo "    nix.nixPath = [ \"wsl-config=$(nix-instantiate --eval -E '<wsl-config>' 2> /dev/null || echo '***')\" ];" >&2
        echo >&2
        exit 2
    fi

    wslPath=$(NIX_PATH=${concatStringsSep ":" config.nix.nixPath} nix-instantiate --eval -E '<wsl>') || true
    if ! test -e "$wslPath"; then
        echo "[1;31merror: Changed <wsl> but target does not exist, aborting activation[0m" >&2
        echo "Add the wsl repo as a channel or set nix.nixPath:" >&2
        exit 2
    fi
